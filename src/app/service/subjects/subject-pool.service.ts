...
export class SubjectPoolService {
  ...
  triggerSubject(name: string, value: any): void {
    const subjectItem = this.subjectsPool$.find(item => item.getName() === name);
    subjectItem.getSubject()
      .next(value);
  }

  removeSubject(name: string): void {
    const subjectItemIdx = this.subjectsPool$.findIndex(item => item.getName() === name);
    if (subjectItemIdx) {
